  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Bangui Fabrication SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Bangui Fabrication SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest reserved-market-ineligible-is-held-and-unoverridable
  (testing "declared :reserved-market? true but workforce composition/legal form fails eligibility -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:reserved-market-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest dgid-record-unverified-is-held-and-unoverridable
  (testing "unverified DGID tax record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:dgid-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
