...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CAF, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up reserved-market-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (reserved-market-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up dgid-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (dgid-record-unverified -> HARD hold) ==")
